...
- $g_{k}^{ghp}$ 表示地源热泵在 $k$ 时刻的产热量
- $p_{k}^{ghp}$ 表示地源热泵在 $k$ 时刻的耗电量
- $k_{k}^{ghp}$ 表示制热效率系数，受到地热井出水温度 $t_{k}^{gtw,out}$ 的影响
...
这里, 
- $g_{k}^{gtw}$ 是地热井的换热量
- $m_{k}^{gtw}$ 代表地热井中流体的质量
- $t_{k}^{gtw,out}$ 为地热井出口温度
- $t_{k}^{gtw,in}$ 为地热井进口温度
- $t_{k}^{b}$ 为地层基本温度
- $k_k^{b}$ 是与流量相关的系数。
...
此式利用 G 函数进行卷积计算，描述了过去地热井换热对现在地温的影响。
- $t^{g}$ 为地层初始温度
- $\kappa$ 为地层的热导率
- $n$ 为地热井的数量
- $L$ 为地热井深度
- $g_{i}^{gtw} 为地热井在第 $i$ 时刻的换热量。
